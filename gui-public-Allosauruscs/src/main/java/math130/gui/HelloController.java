...
// Assuming: Row 1 = Opponent Monsters, Row 2 = Player Monsters
    ...
    private void playSoundEffect(String soundFileName) {
        try {
            ...
            java.net.URL soundURL = getClass().getResource("/sounds/" + soundFileName);
    ...
    // Updated helper method to refresh all deck size texts at once
...
//    @FXML
//    protected void onCardDraw() {
//        Node TopDeck = yourDeck.getChildren().remove(yourDeck.getChildren().size() - 1);
//        yourHand.getChildren().add(TopDeck);
//    }

    @FXML
    private void shuffleDeck() {
        // 1. Get all card nodes currently in the stack
        ArrayList<Node> cards = new ArrayList<>(yourDeck.getChildren());

        // 2. Randomize the list order using Collections
        Collections.shuffle(cards);

        // 3. Re-order the Z-Index in the StackPane to match the shuffled list
        for (Node card : cards) {
            card.toFront(); // Puts the last processed card to the top of the stack
        }
    }
...
private ImageView selectedCardView = null;
    ...
    private void executeAutomaticDrawPhase() {
        ...
        ImageView topCardView = deckPile.remove(deckPile.size() - 1);
        yourHand.getChildren().add(topCardView);
        playSoundEffect("sounds/heehee.mp3");
        if (deckPile.isEmpty() && deckVisualPile != null) {
            deckVisualPile.setImage(null);
